...
const pathSelector =
  'xpath=//iframe[starts-with(@src,"https://plus.espn.com/en/paywall")]';
...
export const getStreamData = async (eventId: string) => {
  ...
  page
    .goto(`https://www.espn.com/espnplus/player/_/id/${eventId}`)
    .catch(() => null);

  // Check to see if we need to login manually
  page
    .waitForSelector(pathSelector, { timeout: 0 })
    .then(async () => {
      page
        .$(pathSelector)
        .then(async (selected) => {
          ...
          selected
            ...
            .then(async (frame) => {
              if (frame) {
                ...
                const loginFrame = await (
                  await page.$("#disneyid-iframe")
                ).contentFrame();
                ...
                if (loginFrame) {
                  ...
                  await loginFrame.fill(
                    ...
                    process.env.ESPN_USER
                  );
                  await sleep(1000);
                  await loginFrame.fill(
                    'xpath=//input[@type="password"]',
                    process.env.ESPN_PASS
                  );
                  await sleep(1000);
                  await loginFrame
                    .locator("button", { hasText: "Log In" })
                    .click();
                  await sleep(1000);
                }
              }
